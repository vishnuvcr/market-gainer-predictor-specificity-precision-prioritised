"""
Walk-forward historical backtesting engine with realistic intraday execution,
Top-5 portfolio allocation, strict capital cap (₹20,000 max per trade including fees),
and exact Paytm Money statutory charges.
"""
import math
import numpy as np
import pandas as pd
...
def calculate_paytm_money_charges(buy_price: float, sell_price: float, qty: int) -> Dict[str, float]:
    ...
    Schedule:
      - Brokerage: Lower of Rs. 20 or 0.05% of turnover per order (buy and sell)
      - STT (Securities Transaction Tax): 0.025% on sell turnover
      - Exchange Transaction Charges (NSE): 0.00297% on total turnover
...
class WalkForwardBacktester:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        # 1. Precompute features for all tickers
        ...
        print(f" [*] Total Trading Dates: {total_dates} | Warm-up: {self.warmup_days} | Test Days: {total_dates - self.warmup_days}")
        
        current_ensemble = None
        days_since_last_train = self.retrain_frequency_days
        
        # Walk-forward loop through out-of-sample trading days
        for date_idx in range(self.warmup_days, total_dates):
            ...
            # Periodic model retraining strictly on data up to prev_date (Zero Lookahead)
            ...
            # Generate predictions at the close of prev_date for execution at Open of current_trade_date
            scored_candidates = []
            ...
            # Select Top K stocks with maximum surge probability
            ...
            # Equal capital allocation per trade: current_equity / top_k, capped at max_capital_per_trade (₹20,000)
    ...
    def _export_results(self, summary: Dict[str, Any]):
        ...
        pd.DataFrame(self.daily_pnl_records).to_csv(BASE_DIR / "backtest        sl_distance = min(ATR_SL_MULTIPLIER * atr_val, MAX_SL_PCT * open_p)
                ...
                # Intraday Trade Resolution
                # If both Target & SL were hit in the same day bar, conservatively assume SL was hit first
                if high_p >= target_price and low_p <= stop_price:
                    exit_price = stop_price
                    exit_reason = "Stop Loss (Both Hit)"
                elif high_p >= target_price:
    ...
    # Paytm Money Brokerage: min(20, 0.05%) on buy + sell
    buy_brokerage = min(20.0, 0.0005 * buy_turnover)
    sell_brokerage = min(20.0, 0.0005 * sell_turnover)
    brokerage = buy_brokerage + sell_brokerage
...
class WalkForwardBacktester:
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if total_dates <= self.warmup_days:
            raise ValueError(f"Not enough dates ({total_dates}) for warmup ({self.warmup_days}).")
        ...
        for date_idx in range(self.warmup_days, total_dates):
            ...
            # Periodic walk-forward model retrain on historical data up to prev_date
            if current_ensemble is None or days_since_last_train >= self.retrain_frequency_days:
                print(f" [TRAINING] Walk-forward refit on historical data up to {prev_date.strftime('%Y-%m-%d')}...")
                current_ensemble = self._train_models_up_to(ticker_features, prev_date)
                days_since_last_train = 0
            ...
            # Score all stocks at the close of prev_date to trade on current_trade_date
            scored_candidates = []
            for ticker, fdf in ticker_features.items():
                if prev_date in fdf.index and current_trade_date in fdf.index:
                    prev_row = fdf.loc[prev_date]
            ...
            # Pick top 5 stocks with maximum surge probability
            scored_candidates.sort(key=lambda x: x["prob"], reverse=True)
            top_picks = scored_candidates[:self.top_k]
            ...
            day_net_pnl = 0.0
            day_trades_count = 0
            capital_per_trade = self.current_equity / float(self.top_k)
            ...
            if (date_idx - self.warmup_days) % 15 == 0 or date_idx == total_dates - 1:
                print(f" [BACKTEST] {date_str} | Trades: {day_trades_count} | Net: ₹{day_net_pnl:+8.2f} | Equity: ₹{self.current_equity:,.2f}")
